...
System: Le fichier de test `test_BESSBatteryEnv.py` semble être incomplet, car il se termine abruptement avec `assert env.max_steps “`. Je vais fournir une version corrigée et complète de ce fichier, ainsi que les instructions pour intégrer tous les fichiers dans votre dépôt. Voici les fichiers nécessaires pour ajouter le job `test` à votre workflow, y compris une version complète de `test_BESSBatteryEnv.py`.

### Fichiers à modifier ou à créer

1. **Mettre à jour `.github/workflows/main.yml`**
   Ajoute un job `test` qui exécute `pytest` sur les fichiers de test dans `Backend/tests/`. Le job `build` reste inchangé.

   <xaiArtifact artifact_id="fb227d35-2ad6-4095-95a9-13f15af7df77" artifact_version_id="16820581-2a58-4e80-9f97-891b276984cd" title="main.yml" contentType="text/yaml">
       ...
       steps:
         ...
         - name: Install dependencies
           run: |
             ...
             pip install pytest numpy pandas stable-baselines3 gym
           shell: bash
         - name: Run tests
           run: |
