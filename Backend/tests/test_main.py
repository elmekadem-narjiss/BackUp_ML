...
def mock_influx_data():
    ...
    with patch("influxdb_client.InfluxDBClient", autospec=True) as mock_client:
        ...
        # Mocker les fonctions dépendantes dans app.utils.time_series et app.services.lstm_model
...
async def test_load_data(mock_influx_data):
    ...
    assert isinstance(response.json(), dict), f"Expected dict response, got {type(response.json())}"
